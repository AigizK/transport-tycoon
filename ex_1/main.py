...
class TransportState:
    ...
    def next_step(self, current_time, warehouses, cargos, routes):
        if self.start_time + self.duration > current_time:
            print("- {} drove  CARGO-{}[{} of {} through]".format(self.name,
                                                                  self.cargoid,
        ...
        if self.cargoid >= 0:
            current_warehouse = \
            list(filter(lambda w: w.name == self.end, warehouses))[0]
        ...
        for ci in current_warehouse.cargoids:
            ...
            candidates = find_next_stop_point_by_way(routes,
            ...
            print(
                "- {} take CARGO-{} from {} to {}".format(self.name,
                                                          self.cargoid,
                                                          self.start, self.end))
            return
...
def find_next_stop_point_by_way(routes, current, end, method, move_step):
    ...
    for r in routes:
        ...
        if move_step <= 1:
            newRoutes = list(
                filter(lambda x: x != Route(r.dest, r.source, r.time,
